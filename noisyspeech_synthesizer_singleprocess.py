...
def normalize_segmental_rms(signal, rms, target_level=-25):
    """Normalize the signal to the target RMS level."""
    scalar = 10 ** (target_level / 20) / (rms + EPS)
    return signal * scalar


def active_rms(clean, noise):
    """Calculate the active RMS values of clean and noise signals."""
    rmsclean = (clean**2).mean() ** 0.5
    rmsnoise = (noise**2).mean() ** 0.5
    return rmsclean, rmsnoise
...
def main_gen(params):
    ...
    while file_num <= params["fileindex_end"]:
        ...
        my_rir = os.path.normpath(
            os.path.join("datasets", "impulse_responses", params["myrir"][rir_index])
        )
        ...
        reverb_clean = add_pyreverb(clean, samples_rir_ch)

        # Normalize and process the reverberated clean audio
        reverb_clean = reverb_clean / (max(abs(reverb_clean)) + EPS)
        rms_reverb_clean = (reverb_clean**2).mean() ** 0.5
        reverb_clean = normalize_segmental_rms(
            reverb_clean, rms=rms_reverb_clean, target_level=-25
        )

        # Ensure no clipping occurs
        if is_clipped(reverb_clean):
            reverb_clean_maxamplevel = max(abs(reverb_clean)) / (0.99 - EPS)
            reverb_clean = reverb_clean / reverb_clean_maxamplevel
        ...
        if params["add_noise"]:
            ...
            noise, noise_sf, noise_cf, noise_laf, noise_index = gen_audio(
                False, params, noise_index, len(reverb_clean)
            )
            ...
            clean_snr, noise_snr, noisy_snr, target_level = segmental_snr_mixer(
                params=params, clean=reverb_clean, noise=noise, snr=snr
            )
            ...
            audiowrite(noisypath, noisy_snr, params["fs"])
            ...
            reverbfilename = (
                clean_files_joined + "_reverb_fileid_" + str(file_num) + ".wav"
            )
            reverbpath = os.path.join(params["noisyspeech_dir"], reverbfilename)
            audiowrite(reverbpath, reverb_clean, params["fs"])

        # Always save the clean audio
        cleanfilename = "clean_fileid_" + str(file_num) + ".wav"
        cleanpath = os.path.join(params["clean_proc_dir"], cleanfilename)
        audiowrite(cleanpath, clean, params["fs"])
...
def main_body():
    ...
    # ADD NOISE OR NOT
    params["add_noise"] = utils.str2bool(cfg["add_noise"])
